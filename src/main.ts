...
const chooseMarkerButton = document.getElementById(
  "choose-marker-btn"
...
const clearMazeButton = document.getElementById(
  "clear-maze-btn"
) as HTMLButtonElement;
...
const startAlgorithmButton = document.getElementById(
  "start-algo-btn"
) as HTMLButtonElement;
...
const getCellById = (id: CellId): Cell | undefined => {
...
startAlgorithmButton.onclick = () => {
  if (
    ...
    showMessage(
      "Please set start and end markers before starting the algorithm."
    );
    return;
  }
...
algorithmSelect.onchange = (e) => {
  ...
  const algo = target.value as CanvasState["algorithmName"];
  canvasState.algorithmName = algo;
};

const getMousePosOnCanvas = (e: MouseEvent) => {
...
const getCellFromMousePos = (mousePos: {
  ...
  const cellId: CellId = `w${col}-h${row}`;
  ...
  canvas.addEventListener("mousedown", (e) => {
    ...
    if (canvasState.state === "drawing_maze") {
      ...
      if (canvasState.markerPos) {
        const { start, end } = canvasState.markerPos;
        if (
          (start && cell.pos.row === start.row && cell.pos.col === start.col) ||
...
const run = () => {
  renderCanvas();
  showMessage(
    canvasState.message || "Welcome to the Graph Algorithm Visualizer!"
  );
